...
if [ -e "$work_dir/second_match/*.second" ]; then
	for i in `ls $work_dir/second_match/*.second`; do
  	  ...
  	  blastn -query ${second}_unfiltered.fasta -out ${second}_ass_index.blast -db $work_dir/${prefix}_ref_index -outfmt "6 qaccver saccver pident qlen slen length mismatch gapopen qstart qend sstart send evalue bitscore" ; # connect the contigs without overlapping reads, based on reference
