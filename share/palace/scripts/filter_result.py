...
def contains_gen(line,genes_seg):
    ...
    for item in line_arr:
        if item in genes_seg:
            return True
    return False
def max_score(line,phagescore):
    max_score = 0
    strip_line = line.strip().replace("+","").replace("-","")
    line_arr = strip_line.split("\t")
    for item in line_arr:
        if item in phagescore.keys():
            if max_score < phagescore[item]:
                max_score = phagescore[item]
    return max_score
...
for idx, line in enumerate(orderin.readlines()):
    ...
    if all_len != 0 and (float(blast_len) / float(all_len)) > 0.2:
        flags = True
    if not flags and all_len < 1000:
        flags = False
    if not flags and (max_score(line,phagescore) < 0.9 or all_len < 1000):
        continue
    for t in tmp:
        if t[-1] == '-' or t[-1] == '+':
            tmp_seq = record_dict[t[0:-1]].seq
        else:
            tmp_seq = record_dict[t].seq
        if t[-1] == '-':
            tmp_seq = record_dict[t[0:-1]].seq.reverse_complement()
        seq = seq + tmp_seq
...
with open(sys.argv[8], 'w') as res:
    s_len=0
    for s in res_count:
        sresult = s.replace('self','').replace('gene','').replace('score','')
        s_len = line_len(s)
        if s_len >= 10000:
            res.write(sresult+"\n")
