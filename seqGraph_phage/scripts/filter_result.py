...
for idx, line in enumerate(orderin.readlines()):
    ...
    if len(tmp) == 1 and self_tag:
        ...
        #     continue
        if contains_gen(line, genehit) or phagescore[idx + 1] >= 0.7:
            for t in tmp:
    ...
    if all_len < 1000:
        flags = False
    if not flags and phagescore[idx + 1] < 0.9 or all_len < 1000:
        continue
    for t in tmp:
        tmp_seq = record_dict[t[0:-1]].seq
        if t[-1] == '-':
            tmp_seq = record_dict[t[0:-1]].seq.reverse_complement()
        seq = seq + tmp_seq
    if contains_gen(line, genehit) and phagescore[idx + 1] >= 0.9:
